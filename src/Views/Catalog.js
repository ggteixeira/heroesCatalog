...
function Catalog() {
  ...
  useEffect(() => {
    fetch(
      `https://gateway.marvel.com/v1/public/characters?ts=${timeStamp}&apikey=${apiKey}&hash=${md5}&orderBy=name&limit=10`
    )
      ...
      {characterInfos.map((hero) => (
        <div className="bg-gray-500 max-w-sm m-3 rounded overflow-hidden shadow-lg">
          <img
            ...
            src={hero.thumbnail.path + "/standard_xlarge.jpg"}
            alt="Thumb not available"
          ></img>
          <div class="font-bold text-sm m-2">
            <p className="text-center">{hero.name}</p>
          </div>
          <p className="hover:bg-gray-600 border text-center border-gray-400 px-4 py-2 text-gray-800">
